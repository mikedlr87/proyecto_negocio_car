...
public:
    manejoVehiculo(): numVehiculos(0), vehiculos(nullptr) {
        cargarVehiculos();
    }
    ...
    void cargarVehiculos() {
        ...
        vehiculos = new Vehiculo[count];
        numVehiculos = 0;

        getline(file, line); 
        while (getline(file, line) && numVehiculos < count) {
    ...
    void eliminarVehiculoPorPlaca(const string& placa) {
        ...
        if (index != -1) {
            ...
            numVehiculos--;
            cout << "Se elimino el vehiculo de placa " << placa << "\n";
        } else {
            cout << "No se encontro el vehiculo de placa " << placa << "\n";
        }
    }
    ...
    void agregarVehiculo() {
        ...
        // Reasignar memoria para agregar un nuevo vehiculo
        Vehiculo* newArray = new Vehiculo[numVehiculos + 1];
        for (int i = 0; i < numVehiculos; ++i) {
            newArray[i] = vehiculos[i];
        }
        delete[] vehiculos;
        vehiculos = newArray;
