#ifndef PARTS_CONTROLLER_H
#define PARTS_CONTROLLER_H
...
public:
    manejoRepuestos() : numRepuestos(0), capacidadRepuestos(10) {
        repuestos = new Repuesto[capacidadRepuestos];
        cargarRepuestos();
    }
    ...
    void cargarRepuestos() {
        ...
        while (getline(file, line)) {
            if (numRepuestos == capacidadRepuestos) {
...
private:
    void redimensionarArreglo() {
        capacidadRepuestos *= 2; 
        Repuesto* nuevoArreglo = new Repuesto[capacidadRepuestos];
        for (int i = 0; i < numRepuestos; ++i) {
            nuevoArreglo[i] = repuestos[i];
        }
        delete[] repuestos; 
        repuestos = nuevoArreglo; 
    }
